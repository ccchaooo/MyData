...
jQuery(function() {
    ...
    MdiWindow.prototype.load = function(sUrl,/*Window*/oWin, callback){
        ...
        if(null != this.left && null != this.top ){
        	 loc_offset = [this.top + 'px', this.left + 'px'];
        }
        if(null != this.top  && null == this.left ){
 ...
 * @param {} message
 */
function openMdiAlert(title, message, icon, callback){
	  var disptitle = "";
	  var iicon = 2;
	  if(null == title || "" == title){
	  	disptitle = "提示";
 ...
 * @param {} okfun
 */
function openMDIconfirm(title, message, cancelfun, returnfun, okfun){
    var disptitle = "";
    if(null == title || "" == title){
      disptitle = "提示";
    }else{
