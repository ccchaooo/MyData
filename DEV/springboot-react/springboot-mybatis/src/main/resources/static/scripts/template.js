$(function () {
    ...
    myGrid.setHeader("序号,房间号,入住时间,退房时间,金额,税费,交易方式,备注");
    myGrid.setInitWidthsP("4,7,12,12,12,12,12,*");//设置列宽（按百分比）
    myGrid.setColAlign("center,left,center,center,center,center,center,center");//设置各列的对齐方式
    myGrid.setColTypes("ro,ro,ro,ro,ro,ro,ro,ro");//设置各列的数据类型
    myGrid.enableResizing("false,true,true,true,false,false,false,false");
    myGrid.enableAlterCss("GridRowColorEven", "GridRowColorUnEven");//设置行的交替样式
    // myGrid.setColumnIds("'',c_rid,d_checkdin,d_checkout,n_total,D_SYYWCBJSJ,platename,comments");
    myGrid.init();
    ...
    myGrid.attachFooter(
        ["<div><span>往下滚动自动加载，总共<span id='foot_zbs'>0</span>笔。</span></div>", "#cspan", "#cspan", "#cspan", "#cspan", "#cspan", "#cspan", "#cspan"],
        ...
        $.post("../query",
            function (data, status) {
                ...
                if (status === "success") {
                    ...
                    //列名
                    var keyarr = ("roomname,d_checkdin,d_checkout,n_theoreticalincome,fee,platename,comments").split(",");
